...
## 下一步
1. 用作業牆管理（`/portal`）建一面攝影測試牆（區段＝① 微距／② 人物／③ 風景，也順便驗收「90 秒內建好、區段一致」）→ 到 Padlet 做三項待辦 → 用學生帳號在各區段各上傳一張自己拍的照片 → 「抓作業＋寫草稿」→ 自己核准、按「發布」，確認評語出現在各區段那一篇底下
2. ~~確認 RDQ 規格卡~~ 2026-09-18 已 confirmed 並實作（`/portal`，grader/portal.py＋static/portal.html）。剩下：真的建一面牆驗收（90 秒內、欄位一致），並重開 8765 的審核頁讓老師用到新版
